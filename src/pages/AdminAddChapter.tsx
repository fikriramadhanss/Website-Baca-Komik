...
import { ChevronLeft, Upload, Save, PlusCircle } from "lucide-react";
...
export default function AdminAddChapter() {
  ...
  const [chapterNumber, setChapterNumber] = useState(comic ? comic.chapters + 1 : 1);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Simulate uploading images and saving chapter data
      ...
      >
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-primary font-bold text-sm mb-1">{comic.subtitle || "Tambah Chapter Baru"}</p>
            <p className="text-gray-400 text-xs">Saat ini memiliki {comic.chapters} Chapter</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Nomor Chapter *</label>
            <input 
              type="number" 
              min="1"
              required
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Upload Gambar Chapter (Halaman Komik) *</label>
            <div className="border-2 border-dashed border-white/10 rounded-lg p-10 flex flex-col items-center justify-center text-center hover:border-primary/50 transition-colors cursor-pointer bg-dark relative">
              ...
              <span className="text-base text-gray-300 font-medium">Pilih gambar-gambar untuk chapter ini</span>
            ...
            {chapterImages && chapterImages.length > 0 && (
              <p className="text-xs text-green-400 mt-2">✓ {chapterImages.length} file gambar dipilih.</p>
            )}
          </div>
