...
export default function AdminEditChapter() {
  ...
  if (!comic || !chapterId) {
    return (
      <div className="text-center py-20 text-white">
        <p>Komik atau Chapter tidak ditemukan.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      <Link to={`/admin/edit/${comic.id}`} className="text-gray-400 hover:text-white flex items-center gap-2 mb-6 w-fit transition-colors">
        <ChevronLeft size={20} />
        <span>Kembali ke Edit Komik</span>
      </Link>
      ...
      >
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black text-white line-clamp-1">{comic.title}</h1>
            <p className="text-primary font-bold text-sm mb-1">{comic.subtitle || "Karya Original"}</p>
            <p className="text-gray-400 text-xs">Edit Chapter {chapterId}</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Upload Ulang Gambar Chapter *</label>
            <div className="border-2 border-dashed border-white/10 rounded-lg p-10 flex flex-col items-center justify-center text-center hover:border-primary/50 transition-colors cursor-pointer bg-dark relative">
              ...
              <span className="text-base text-gray-300 font-medium">Pilih gambar-gambar baru untuk chapter ini</span>
              <span className="text-sm text-gray-500 mt-1">Ini akan menimpa gambar sebelumnya. Bisa pilih banyak file sekaligus (Multiple)</span>
              <input 
                type="file" 
                multiple 
                required
            ...
            {chapterImages && chapterImages.length > 0 && (
              <p className="text-xs text-green-400 mt-2">✓ {chapterImages.length} file gambar baru dipilih siap diupload.</p>
            )}
          </div>
          ...
          <div className="flex justify-end pt-4 border-t border-white/5">
            ...
            >
              <Save size={20} /> Simpan Perubahan Chapter
            </button>
          </div>
        </form>
      </motion.div>
